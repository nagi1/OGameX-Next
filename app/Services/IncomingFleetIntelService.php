...
class IncomingFleetIntelService
{
    ...
    /**
     * Apply espionage-based redaction to a hostile foreign fleet event row.
     */
    ...
    /**
     * Shape a foreign friendly or neutral fleet without espionage redaction.
     *
     * Commander support is not implemented yet, so hasCommander() currently returns false.
     * Keeping the decision here ensures the visibility rule is ready when officers land.
     */
    public function applyFriendly(FleetEventRowViewModel $row, bool $showShipment): void
    {
        $row->fleet_intel_level = IncomingFleetIntelLevel::Full;
        $row->show_shipment = $showShipment;
    ...
    public function applyToUnionSummary(FleetEventRowViewModel $summaryRow, IncomingFleetIntelLevel $level, int $viewerUserId): void
    {
        ...
        $summaryRow->fleet_units = $level->showsShipTypes()
            ? $this->mergeUnionFleetUnits($summaryRow->union_member_fleets)
            : new UnitCollection();
    ...
    /**
     * Merge visible member fleet collections for a union summary tooltip.
     *
     * @param array<FleetEventRowViewModel> $memberFleets
     */
    private function mergeUnionFleetUnits(array $memberFleets): UnitCollection
    {
        $units = new UnitCollection();

        foreach ($memberFleets as $memberFleet) {
            $units->addCollection($memberFleet->fleet_units);
        }
    ...
    private function redactUnionBreakdownShipCounts(FleetEventRowViewModel $row): void
    {
        foreach ($row->union_player_breakdown as $playerIndex => $playerInfo) {
            foreach ($playerInfo['origins'] as $originIndex => $origin) {
                $row->union_player_breakdown[$playerIndex]['origins'][$originIndex] = [
                    'planet_name' => $origin['planet_name'],
                    'coords' => $origin['coords'],
                    'fleet_count' => $origin['fleet_count'],
                    'ship_count' => 0,
                ];
            }
        }
