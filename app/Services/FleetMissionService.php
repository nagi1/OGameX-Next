...
class FleetMissionService
{
    ...
    public function syncMissionArrivalJobs(FleetMission $mission): void
    {
        ...
        if ($physicalArrivalTime !== null) {
            $holdJobId = $this->resolveOrReplaceJob($mission->id, $mission->hold_job_id, $physicalArrivalTime, true);
        } else {
     ...
     */
    private function resolveOrReplaceJob(int $missionId, int|null $existingJobId, int $triggerTime, bool $isHoldArrival = false): int|null
    {
        ...
        return $this->dispatchMissionArrivalJob($mission, $triggerTime, $isHoldArrival);
    }
     ...
     */
    public function arrivalQueueForMission(FleetMission $mission, bool $isHoldArrival = false): string
    {
     ...
     */
    private function dispatchMissionArrivalJob(FleetMission $mission, int $triggerTime, bool $isHoldArrival = false): int|null
    {
        ...
        $jobId = Queue::laterOn(
            $this->arrivalQueueForMission($mission, $isHoldArrival),
