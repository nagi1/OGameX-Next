...
class IncomingFleetEspionageIntelTest extends FleetDispatchTestCase
{
    ...
    /**
     * Fleet movement must use the same hostile-fleet redaction as the event list.
     */
    public function testFleetMovementHidesIncomingHostileFleetAtEspionageZero(): void
    {
        $response = $this->fetchDefenderPageWithIncomingAttack(0, 13, '/fleet/movement');
    ...
    {
        return $this->fetchDefenderPageWithIncomingAttack(
            $defenderEspionageLevel,
            $shipCount,
            '/ajax/fleet/eventlist/fetch'
        );
    }

    /**
     * Send an attack from the current user, then load a defender page for the given spy level.
     */
    private function fetchDefenderPageWithIncomingAttack(
        ...
        if ($defenderPlayer === null) {
            throw new RuntimeException('Foreign planet has no player.');
        }
