...
class FleetArrivalQueueTest extends FleetDispatchTestCase
{
    ...
    public function testDeletingMissionRemovesPendingArrivalJobs(): void
    {
        ...
        $mission = FleetMission::query()->where('user_id', $this->currentUserId)->whereNull('parent_id')->latest('id')->firstOrFail();
        $arrivalJobId = $mission->arrival_job_id;
        $this->assertNotNull($arrivalJobId);

        $mission->delete();

        $this->assertFalse(DB::table('jobs')->where('id', $arrivalJobId)->exists());
    }

    public function testUnchangedMissionReusesItsArrivalJob(): void
    {
        $this->basicSetup();
        $this->sendMissionToSecondPlanet($this->createCargoUnits(1), new Resources(1000, 500, 250, 0));
        ...
        $mission->refresh();
        $this->assertSame($originalJobId, $mission->arrival_job_id);
        $this->assertSame(1, DB::table('jobs')->where('id', $originalJobId)->count());
    }

    public function testChangedArrivalTimeReplacesPendingJob(): void
    {
        $this->basicSetup();
        $this->sendMissionToSecondPlanet($this->createCargoUnits(1), new Resources(1000, 500, 250, 0));
        ...
        $mission->time_arrival += 60;
        $mission->save();
        $mission->refresh();

        $this->assertNotSame($originalJobId, $mission->arrival_job_id);
        $this->assertFalse(DB::table('jobs')->where('id', $originalJobId)->exists());
        $this->assertSame((int) $mission->time_arrival, (int) DB::table('jobs')->where('id', $mission->arrival_job_id)->value('available_at'));
    }

    public function testCoordinateDestinationLockKeyDoesNotRequirePlanetId(): void
    {
        $mission = new FleetMission();
        ...
        $this->assertSame('fleet-destination:coords:1:1:2:3', resolve(FleetMissionService::class)->getMissionDestinationLockKey($mission));
    }
